...
string GetDirectoryPathAgain(string sFilePath)
{
	// Get directory path
	string sDirectory = "";
...
bool CObjModel::LoadModel(string sFileName, string sMtlFileName)
{
	...
	if(iAttrBitField&2)
	LoadMaterial(GetDirectoryPathAgain(sFileName)+sMtlFileName);
...
bool CObjModel::LoadMaterial(string sFullMtlFileName)
{
	...
	while(fgets(line, 255, fp))
	{
		...
		if(sType == "map_Ka")
		{
			...
			tAmbientTexture.LoadTexture2D(GetDirectoryPathAgain(sFullMtlFileName)+sTextureName, true);
